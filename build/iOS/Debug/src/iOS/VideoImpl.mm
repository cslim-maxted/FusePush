...
namespace FuseVideoImpl
{
	...
	void freeVideoState(void * videoState)
	{
		...
		if (vs->PlayerItem)
		{
			[vs->PlayerItem release];
			vs->PlayerItem = NULL;
		}

		if (vs->PlayerItemVideoOutput)
		{
			[vs->PlayerItemVideoOutput release];
	...
	{
		VideoState * vs = (VideoState*)videoState;
		return (int)vs->WidthCache;
	}

	int getHeight(void * videoState)
	{
		VideoState * vs = (VideoState*)videoState;
		return (int)vs->HeightCache;
	}
	...
	int updateTexture(void * videoState)
	{
		...
		vs->WidthCache = CVPixelBufferGetWidth(pixelBuffer);
		vs->HeightCache = CVPixelBufferGetHeight(pixelBuffer);
		...
		CVReturn err = CVOpenGLESTextureCacheCreateTextureFromImage(
			...
			GL_TEXTURE_2D,
			GL_RGBA,
			(GLsizei)vs->WidthCache,
			(GLsizei)vs->HeightCache,
			GL_BGRA_EXT,
			GL_UNSIGNED_BYTE,
			0,
			&vs->TextureHandle);
